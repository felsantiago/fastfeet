...
export default function Delivery() {
  ...
  const loadDeliveries = async () => {
    try {
      const { data, headers } = await api.get('delivery', {
        params: { page, name: filter },
      });

      setTotalPages(Math.ceil(headers['x-total-count'] / 7));
      setDeliveries(
        data.map((name) => ({
          ...name,
          start_date: name.start_date
            ? format(parseISO(name.start_date), 'dd/MM/yyyy')
            : null,
          end_date: name.end_date
            ? format(parseISO(name.end_date), 'dd/MM/yyyy')
            : null,
        }))
      );
    } catch (err) {
